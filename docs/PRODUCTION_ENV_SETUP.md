# Настройка .env файлов для продакшена

## 📁 Структура .env файлов

На продакшене используется **один .env файл в корне проекта**, который читается docker-compose.yml.

```
PREPODAVAI/
├── .env                    # ← Главный файл для продакшена (НЕ коммитьте в git!)
├── docker-compose.yml      # Читает переменные из .env
├── backend/
│   └── env.example         # Пример для разработки
└── ...
```

## 🔧 Создание .env файла

### Шаг 1: Создайте .env в корне проекта

```bash
cd /path/to/PREPODAVAI
cp docs/PRODUCTION_ENV_EXAMPLE .env
# Или создайте вручную
nano .env
```

### Шаг 2: Заполните все переменные

См. пример ниже.

## 📝 Пример .env для продакшена

```bash
# ============================================
# Database
# ============================================
POSTGRES_USER=prepodavai
POSTGRES_PASSWORD=ВАШ_СИЛЬНЫЙ_ПАРОЛЬ_ДЛЯ_POSTGRES
POSTGRES_DB=prepodavai

# ============================================
# Redis
# ============================================
REDIS_PASSWORD=ВАШ_СИЛЬНЫЙ_ПАРОЛЬ_ДЛЯ_REDIS

# ============================================
# Backend API
# ============================================
NODE_ENV=production
PORT=3001
JWT_SECRET=ВАШ_СИЛЬНЫЙ_JWT_СЕКРЕТ_МИНИМУМ_32_СИМВОЛА

# CORS - укажите ваш домен
CORS_ORIGIN=https://yourdomain.com,https://www.yourdomain.com

# Администраторы (ID пользователей через запятую)
ADMIN_USER_IDS=user-id-1,user-id-2

# ============================================
# Telegram Bot
# ============================================
TELEGRAM_BOT_TOKEN=ВАШ_TELEGRAM_BOT_TOKEN
NEXT_PUBLIC_TELEGRAM_BOT_NAME=your_bot_name

# ============================================
# External Services
# ============================================
N8N_WEBHOOK_URL=https://your-n8n-instance.com/webhook/
OPENAI_API_KEY=your_openai_key_if_needed
GAMMA_API_KEY=your_gamma_key_if_needed

# ============================================
# Frontend
# ============================================
NEXT_PUBLIC_API_URL=https://api.yourdomain.com
# Или если API на том же домене:
# NEXT_PUBLIC_API_URL=https://yourdomain.com/api

# ============================================
# GigaChat API (ОБЯЗАТЕЛЬНО!)
# ============================================
GIGACHAT_CLIENT_ID=ваш_gigachat_client_id
GIGACHAT_CLIENT_SECRET=ваш_gigachat_client_secret
GIGACHAT_SCOPE=GIGACHAT_API_PERS

# URL для авторизации (обычно не меняется)
GIGACHAT_AUTH_URL=https://ngw.devices.sberbank.ru:9443

# URL API (обычно не меняется)
GIGACHAT_API_URL=https://gigachat.devices.sberbank.ru/api/v1

# Endpoint для изображений (обычно не меняется)
GIGACHAT_IMAGE_ENDPOINT=/images/edits

# ВАЖНО: На продакшене ВСЕГДА должно быть false!
GIGACHAT_DISABLE_TLS_VERIFICATION=false

# Модели по умолчанию (можно оставить как есть)
GIGACHAT_DEFAULT_CHAT_MODEL=GigaChat
GIGACHAT_DEFAULT_IMAGE_MODEL=GigaChat-Image
GIGACHAT_DEFAULT_EMBEDDINGS_MODEL=GigaChat-Embedding
GIGACHAT_DEFAULT_AUDIO_MODEL=GigaChat-Audio
```

## 🔐 Генерация сильных паролей и секретов

### JWT_SECRET (минимум 32 символа)
```bash
openssl rand -hex 32
```

### POSTGRES_PASSWORD
```bash
openssl rand -base64 24
```

### REDIS_PASSWORD
```bash
openssl rand -base64 16
```

## ✅ Проверка .env файла

### 1. Проверьте, что все обязательные переменные заполнены

```bash
# Проверка обязательных переменных
grep -E "^[A-Z_]+=" .env | grep -v "^#" | wc -l
```

### 2. Проверьте синтаксис

```bash
# Не должно быть ошибок
set -a
source .env
set +a
```

### 3. Проверьте, что файл не в git

```bash
# Добавьте в .gitignore (если еще не добавлено)
echo ".env" >> .gitignore

# Проверьте, что .env не отслеживается
git check-ignore .env
# Должно вернуть: .env
```

## 🚀 Применение изменений

После изменения .env файла:

```bash
# Перезапустите контейнеры
docker compose down
docker compose up -d

# Или только backend
docker compose restart backend
```

## 🔍 Проверка переменных в контейнере

```bash
# Проверить переменные GigaChat
docker exec prepodavai-backend-prod env | grep GIGACHAT

# Проверить все переменные
docker exec prepodavai-backend-prod env | sort
```

## ⚠️ Важные замечания

1. **НЕ коммитьте .env в git!** Файл содержит секреты
2. **Используйте сильные пароли** для всех сервисов
3. **GIGACHAT_DISABLE_TLS_VERIFICATION=false** на продакшене
4. **JWT_SECRET** должен быть минимум 32 символа
5. **CORS_ORIGIN** должен содержать ваш реальный домен
6. **Храните .env в безопасном месте** (например, в менеджере секретов)

## 📋 Чек-лист перед деплоем

- [ ] .env файл создан в корне проекта
- [ ] Все переменные заполнены реальными значениями
- [ ] JWT_SECRET сгенерирован и минимум 32 символа
- [ ] POSTGRES_PASSWORD сильный и уникальный
- [ ] REDIS_PASSWORD сильный и уникальный
- [ ] GIGACHAT_CLIENT_ID и GIGACHAT_CLIENT_SECRET заполнены
- [ ] GIGACHAT_DISABLE_TLS_VERIFICATION=false
- [ ] CORS_ORIGIN содержит ваш домен
- [ ] NEXT_PUBLIC_API_URL указывает на правильный API URL
- [ ] .env добавлен в .gitignore
- [ ] .env не отслеживается git (проверьте `git status`)

## 🆘 Если что-то не работает

### Проблема: Переменные не применяются

```bash
# 1. Проверьте, что .env в корне проекта
ls -la .env

# 2. Проверьте синтаксис
cat .env | grep -v "^#" | grep -v "^$"

# 3. Перезапустите контейнеры
docker compose down
docker compose up -d
```

### Проблема: "GIGACHAT_CLIENT_ID is required"

```bash
# Проверьте переменные в контейнере
docker exec prepodavai-backend-prod env | grep GIGACHAT

# Если переменных нет, проверьте docker-compose.yml
# Убедитесь, что они добавлены в environment секцию backend
```

### Проблема: Секреты видны в логах

```bash
# Проверьте логи
docker logs prepodavai-backend-prod | grep -i secret

# Если секреты видны, проверьте код - не логируйте секреты!
```

