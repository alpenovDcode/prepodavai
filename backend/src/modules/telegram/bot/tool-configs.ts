...
export interface FieldConfig {
  ...
  default?: string;
  options?: FieldOption[];
  // Список опций, зависящий от уже собранных params (для exam-variant subject)
  conditionalOptions?: (params: Record<string, string>) => FieldOption[];
  maxLength: number;
  skipLabel?: string;
  // Только для type: 'file'
  accept?: 'photo' | 'document';  // photo = ctx.message.photo, document = ctx.message.document
  maxSizeMb?: number;             // Лимит по размеру (Telegram Bot API: max 20 МБ через getFile)
  storeAs?: 'hash' | 'url';      // Что сохранять из ответа FilesService: hash или url
}
...
// ── Уровни для worksheet / content-adaptation ────────────────────────────────
...
export const TOOL_CONFIGS: ToolConfig[] = [
  // ── 1. Рабочий лист ─────────────────────────────────────────────────────────
  {
    ...
    fields: [
      {
        key: 'subject',
        label: '📚 Предмет (необязательно)\n\nНапример: Математика, История, Биология',
        type: 'text',
        required: false,
        maxLength: 100,
      ...
      {
        key: 'topic',
        label: '✏️ Тема урока\n\nНапример: Квадратные уравнения, Первая мировая война',
        type: 'text',
        required: true,
        maxLength: 200,
      },
      {
        key: 'level',
        label: '🎓 Уровень учеников',
        type: 'select',
        required: true,
        default: 'Средняя школа',
      ...
      },
      {
        ...
        default: '10',
        options: [
          { value: '5', label: '5 заданий' },
          { value: '10', label: '10 заданий' },
          { value: '15', label: '15 заданий' },
          { value: '20', label: '20 заданий' },
        ],
        maxLength: 3,
      },
    ],
  ...
  // ── 2. Тест ──────────────────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      {
        key: 'answersCount',
        label: '✅ Вариантов ответа на каждый вопрос',
        type: 'select',
        required: true,
        default: '4',
        options: [
  ...
  // ── 3. Словарь ───────────────────────────────────────────────────────────────
  {
    ...
    fields: [
      {
        key: 'topic',
        label: '✏️ Тема словаря\n\nНапример: Путешествия, Еда, Математические термины',
        type: 'text',
        required: true,
        maxLength: 200,
      },
      ...
      },
      {
        ...
        options: [
          ...
          { value: '15', label: '15 слов' },
          { value: '20', label: '20 слов' },
          { value: '25', label: '25 слов' },
  ...
  // ── 4. Конструктор уроков ────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      {
        key: 'topic',
        label: '✏️ Тема урока\n\nНапример: Теорема Пифагора, Фотосинтез',
        type: 'text',
        required: true,
        maxLength: 200,
      },
      {
        ...
        options: [
          ...
          { value: 'Старшая Школа', label: '9–11 класс' },
        ],
        maxLength: 20,
      },
      {
        key: 'duration',
        label: '⏱️ Длительность урока',
        type: 'select',
        required: true,
        default: '45',
        options: [
      ...
      },
      {
        ...
        options: [
          { value: 'Интерактивный', label: '🎮 Интерактивный' },
    ...
    fields: [
      {
        ...
        default: 'ОГЭ (9 класс)',
        options: [
          { value: 'ОГЭ (9 класс)', label: 'ОГЭ (9 класс)' },
          { value: 'ЕГЭ (11 класс)', label: 'ЕГЭ (11 класс)' },
          { value: 'ВПР', label: 'ВПР' },
        ],
        maxLength: 20,
      },
      {
        ...
        required: true,
        maxLength: 50,
        // Опции зависят от выбранного типа экзамена
  ...
  // ── 6. Изображение ───────────────────────────────────────────────────────────
  {
    ...
    fields: [
      {
        key: 'prompt',
        label: '🖼️ Опишите изображение\n\nНапример: Строение клетки с подписями на русском, Карта Древнего Рима',
        type: 'text',
        required: true,
        maxLength: 500,
      },
      {
        ...
        options: [
          ...
          { value: 'sketch', label: '✏️ Эскиз' },
          { value: 'illustration', label: '🖌️ Иллюстрация' },
          { value: '3d-model', label: '🗿 3D модель' },
          { value: 'anime', label: '⛩️ Аниме' },
        ],
        maxLength: 20,
      },
    ],
  ...
  // ── 7. Игра ──────────────────────────────────────────────────────────────────
  {
    ...
    fields: [
      ...
      {
        key: 'topic',
        label: '✏️ Тема игры\n\nНапример: История Древнего Рима, Таблица умножения',
        type: 'text',
        required: true,
        maxLength: 200,
      },
  ...
  // ── 8. Презентации ──────────────────────────────────────────────────────────
  {
    ...
    fields: [
      {
        key: 'topic',
        label: '✏️ Тема презентации\n\nНапример: Фотосинтез, Первая мировая война, Python для начинающих',
        type: 'text',
        required: true,
        maxLength: 300,
      },
      {
        key: 'duration',
        label: '⏱️ Длительность выступления',
        type: 'select',
        required: true,
        default: '15',
        options: [
      ...
      },
      {
        ...
        options: [
          ...
          { value: 'academic', label: '📚 Строгий (академический)' },
          { value: 'creative', label: '🌈 Яркий (творческий)' },
          { value: 'corporate', label: '💼 Деловой (корпоративный)' },
        ],
        maxLength: 20,
      },
      {
  ...
  // ── 9. Анализ видео ──────────────────────────────────────────────────────────
  {
    ...
    fields: [
      {
        key: 'fileUrl',
        label: '🔗 Ссылка на видео в Яндекс.Диске\n\nОткройте доступ по ссылке и вставьте её сюда.\nПример: https://disk.yandex.ru/i/...',
        type: 'text',
        required: true,
        maxLength: 500,
      },
      {
        ...
        options: [
          { value: 'methodological', label: '📋 Методический анализ урока' },
          { value: 'sales', label: '💰 Анализ пробного урока (продажи)' },
        ],
        maxLength: 20,
      },
    ],
    ...
    fields: [
      ...
      },
      {
        ...
        options: [
          ...
          { value: '1024x1792', label: '📱 Вертикаль (9:16)' },
          { value: '1792x1024', label: '🖥️ Горизонталь (16:9)' },
        ],
        maxLength: 20,
      },
      {
        key: 'prompt',
        label: '🌟 Сценарий фотосессии',
        type: 'select',
        required: true,
        default: 'Деловой портрет',
        options: [
          { value: 'Летний портрет', label: '☀️ Летний портрет' },
          { value: 'Деловой портрет', label: '💼 Деловой портрет' },
          { value: 'Семейная фотосессия', label: '👨‍👩‍👧 Семейная' },
          { value: 'Портрет в студии', label: '🎬 Студийный портрет' },
          { value: 'Романтическая фотосессия', label: '💕 Романтическая' },
          { value: 'Спортивная фотосессия', label: '🏃 Спортивная' },
          { value: 'Детская фотосессия', label: '🧸 Детская' },
          { value: 'Выпускная фотосессия', label: '🎓 Выпускная' },
          { value: 'Портрет в городе', label: '🏙️ Городской портрет' },
          { value: 'На пляже', label: '🏖️ На пляже' },
          { value: 'В библиотеке', label: '📚 В библиотеке' },
          { value: 'Свадебная фотосессия', label: '💍 Свадебная' },
        ],
        maxLength: 50,
      },
    ],
  ...
  // ── 11. Транскрибация ────────────────────────────────────────────────────────
  {
    ...
    fields: [
      {
        key: 'fileUrl',
        label: '🎙️ Отправьте аудио или видео файл\n\nПоддерживается: MP3, WAV, OGG, M4A, MP4, MOV\nМаксимальный размер — 20 МБ.',
        type: 'file',
        required: true,
        accept: 'document',
        maxSizeMb: 20,
