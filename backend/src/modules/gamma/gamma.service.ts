...
export class GammaService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (!this.apiKey) {
      this.logger.warn(
        'GAMMA_API_KEY is not configured. Gamma presentation generation will not work.',
      );
    }
  }
  ...
  async generatePresentation(request: GammaGenerationRequest): Promise<GammaGenerationResponse> {
    ...
    try {
      ...
      const response = await axios.post(`${this.baseUrl}/generations`, request, {
        headers: {
      ...
      if (error.response) {
        this.logger.error(
          `Gamma API error response: ${JSON.stringify(error.response.data, null, 2)}`,
        );
      ...
      if (error.response) {
        ...
        if (error.response.status === 404 || error.response.status === 400) {
          return {
            ...
            status: 'failed',
            error:
              error.response.data?.message || error.response.data?.error || 'Generation not found',
          };
        }
      }
