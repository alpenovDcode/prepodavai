...
=== СТРУКТУРА ЗАГОЛОВКОВ — ВАЖНО ДЛЯ ВЁРСТКИ ===

Рендерер автоматически группирует блоки в карточки на основе heading'ов:

  • heading level 1 ИЛИ heading с текстом «Блок N», «Раздел», «Часть», «Глава»
    → СЕКЦИЯ-разделитель (крупный заголовок с цветной полосой слева).
    Используй для группировки нескольких задач, например «Блок 1: Понятие вектора»,
    «Блок 2: Линейные операции».
  • Любой другой heading (level 2 или 3) → начинает КАРТОЧКУ-задание.
    Заголовок карточки рендерится синим жирным. Под ним — содержимое задания
    (paragraph, multiple-choice, fill-blank, short-answer и т.п.) внутри
    карточки с белым фоном и тонкой рамкой.

Правильная структура worksheet'а:
  1) (опц.) heading level 1 — общее введение
  2) (опц.) paragraph — вступительный текст
  3) heading level 2 «Блок 1: …» — секция-разделитель
  4) heading level 2 «Задание 1. Тема» — карточка задания
  5) блоки задания
  6) heading level 2 «Задание 2. …» — следующая карточка
  ...
  N) (опц.) heading level 2 «Блок 2: …» — следующая секция

Если worksheet короткий и без явного деления на блоки — секцию-разделитель
можно опустить, просто стэк карточек.
