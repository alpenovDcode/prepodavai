...
${customPrompt ? `Дополнительно: ${customPrompt}` : ''}`;
  ...
  private buildWorksheetPrompt(inputParams: Record<string, any>) {
    ...
    const systemPrompt = `Ты — профессиональный методист и верстальщик образовательных материалов. 
Твоя задача: Сгенерировать рабочий лист в формате HTML, который визуально идентичен распечатанному документу формата А4.

ГЛАВНОЕ ТРЕБОВАНИЕ К ВЕРСТКЕ (СТИЛЬ "БУМАЖНЫЙ ЛИСТ"):
1.  **Основа:** Весь контент должен быть обернут в структуру, имитирующую листы А4.
    -   Ширина контента: 210mm.
    -   Минимальная высота: 297mm.
    -   Padding: 20mm.
    -   Фон страницы (body): светло-серый (#f0f0f0).
    -   Фон листа (.sheet): белый (#ffffff).
    -   Шрифт: "Times New Roman", Times, serif (для академического вида) или Inter/Arial (если предмет технический).

2.  **Структура документа (ОБЯЗАТЕЛЬНО):**
    -   **Шапка (на первом листе):**
        -   Слева: "Рабочий лист по предмету: [Предмет]" (жирным).
        -   Справа: Логотип/Текст "Обучай" (или название школы).
        -   Ниже: Поля для заполнения от руки: "Имя: _______", "Класс: _______", "Дата: _______".
    -   **Тело заданий:** Нумерованный список. Задания не должны разрываться посередине при печати (используй page-break-inside: avoid).
    -   **Блок ответов:** Должен начинаться с НОВОЙ СТРАНИЦЫ (используй page-break-before: always).

3.  **Стилизация элементов (КАК В ПРИМЕРЕ):**
    -   **Тесты:** Используй символы квадратов для чекбоксов ($$\\square$$ или HTML entity &#9744;). Пример: "☐ Вариант А".
    -   **Впиши пропущенное:** Используй нижнее подчеркивание или border-bottom. Пример: "Мороз и _______; день чудесный".
    -   **Соотнесение (Match columns):** Используй Flexbox или Grid для создания двух колонок (Левый столбик - Правый столбик).
    -   **Таблицы:** Тонкие границы (border: 1px solid #000), border-collapse: collapse.
    -   **Разлиновка:** Для открытых вопросов добавляй пустые строки для письма (пунктирные или сплошные линии).

4.  **Математические формулы (LaTeX):**
    -   Строчные: $$...$$ (только двойные доллары).
    -   Блочные: $$...$$ (на новой строке).
    -   В <head> вставь скрипт MathJax (конфигурация ниже).

5.  **Технические требования:**
    -   Верни ТОЛЬКО валидный HTML (с <!DOCTYPE html>).
    -   Включи CSS стили внутри <style>.
    -   Для печати добавь @media print { body { background: white; } .sheet { box-shadow: none; margin: 0; width: 100%; } }.

КОНФИГУРАЦИЯ MATHJAX (вставь в <head>):
<script>
window.MathJax = {
...
`;
    ...
    if (questionsCount) details.push(`Количество заданий: ${questionsCount} (Сделай так, чтобы они заняли не менее 1-2 страниц + отдельная страница для ответов)`);
    ...
    const userPrompt = `Сгенерируй полноценный рабочий лист.
Вводные данные:
${details.length ? details.join('\n') : 'Используй стандартные параметры для урока литературы.'}

Структура контента:
1. Шапка (Предмет, "Обучай", Имя/Класс/Дата).
2. Задания (разнообразные: тесты, открытые вопросы, таблицы, соотнесение).
   - Если заданий много, они должны естественным образом перетекать на следующий лист (визуально или через page-break).
3. В самом конце добавь разрыв страницы (<div class="sheet page-break">...</div> или просто начни новый .sheet) для раздела "ОТВЕТЫ".

Убедись, что дизайн строгий, академический и готов к печати на принтере.`;
