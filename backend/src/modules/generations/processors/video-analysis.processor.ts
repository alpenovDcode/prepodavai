...
export class VideoAnalysisProcessor extends WorkerHost {
  ...
  private async generateAnalysis(
    ...
    const systemPrompt =
      type === 'sales'
        ? 'Ты — профессиональный эксперт по продажам в EdTech и ментор для преподавателей. Твоя задача — провести глубокий, критический и конструктивный аудит ПРОБНОГО УРОКА. Ты оцениваешь его как продающий бесплатный урок. Твой тон — экспертный, поддерживающий, но честный. ТВОЯ ЦЕЛЬ: Помочь преподавателю улучшить выявление потребностей и самопрезентацию для повышения конверсий.'
...
---
ТИП АУДИТА: ${type === 'sales' ? '🎯 АНАЛИЗ ПРОБНОГО УРОКА (ПРОДАЖА И ПРЕЗЕНТАЦИЯ)' : '🎓 МЕТОДИКА И КАЧЕСТВО ОБУЧЕНИЯ'}
---

Твоя задача — вернуть **ТОЛЬКО HTML-КОД** готового отчета. Не пиши никаких вступительных слов вроде "Вот ваш отчет". Сразу начинай с тега <div> или заголовков.

СТРУКТУРА ОТЧЕТА И КРИТЕРИИ АНАЛИЗА:

1.  **📊 ЭКСПРЕСС-РЕЗЮМЕ**
    *   Общая оценка (по 10-балльной шкале).
    *   Главное впечатление (2-3 предложения).
...
2.  **🔎 ДЕТАЛЬНЫЙ РАЗБОР ПО БЛОКАМ**
    ...
    *   **🤝 Выявление потребностей:**
        *   Насколько глубоко преподаватель исследовал цели, боли и уровень ученика?
        *   Задавались ли правильные уточняющие вопросы?
    *   **🌟 Самопрезентация преподавателя:**
        *   Как преподаватель преподнес свою экспертность и личный бренд?
        *   Вызывает ли подача доверие и симпатию?
    *   **💡 Презентация методики:**
        *   Понятно ли, как именно ученик придет к результату?
        *   Продана ли ценность обучения именно в этой школе/у этого учителя?
    *   **🎁 Оффер и закрытие:**
        *   Был ли понятный призыв к продолжению (CTA)?
        *   Насколько органично сделано предложение платных занятий?
    `
        : `
    ...
    *   **🤝 Вовлечение и Интерактив:**
        *   Как спикер удерживает внимание? (Вопросы, задания).
    *   **🏁 Закрепление и Рефлексия:**
        *   Было ли подведение итогов и понятен ли следующий шаг?
    `
    }

3.  **💡 ПРАКТИЧЕСКИЕ РЕКОМЕНДАЦИИ (Action Plan)**
    *   Напиши 3-5 конкретных советов: "Что изменить прямо сейчас, чтобы стало лучше".
    *   Приведи примеры удачных фраз для самопрезентации или выявления потребностей.

ТРЕБОВАНИЯ К ОФОРМЛЕНИЮ:
*   Используй **ЭМОДЗИ** 🎨 в заголовках.
*   Используй <h3>, <h4>, <ul>, <li>.
