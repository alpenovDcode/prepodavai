...
Не уходи в смежные темы. Не подменяй тему на родственную. Не давай вопросы «вообще по предмету».
...
2. Каждый вопрос должен проходить тест: «Это вопрос про <MAIN_TOPIC>?». Если нет — переделай.
...
□ Для каждого вопроса задай себе вопрос: «Это вопрос именно по теме <MAIN_TOPIC>${topic || '—'}</MAIN_TOPIC>, а не по смежной?». Если хоть один — нет, перепиши.
□ Подсчитай: сколько вопросов прямо опираются на ключевые понятия темы. Должно быть ≥ 80%.
□ Если ты пишешь вопросы «вообще про предмет ${subject || '—'}», а не про конкретно <MAIN_TOPIC> — остановись и перепиши.
