...
export class MaxService {
  ...
  private async handleMessage(message: any) {
    ...
    try {
      ...
      // Ввод произвольной даты срока сдачи домашнего задания
      const hwState = this.getMaxPlatformState(userIdForDb);
      if (text && hwState.pendingHwTarget) {
        const iso = this.parseRuDate(text);
        if (iso) {
          await this.doAssignHomework(chatId, userIdForDb, 0, iso);
        } else {
          await this.sendMessage(chatId, '❌ Не удалось распознать дату. Попробуйте: *20 июня*, *20.06* или *20/06/2026*.');
        }
        return;
      }
  ...
  private async handleCallback(callback: any) {
    ...
    try {
      ...
      if (payload.startsWith('hw:c:')) {
        ...
        await this.sendMessageWithKeyboard(chatId, '📅 Выберите срок сдачи или напишите дату текстом (например: *20 июня*, *20.06*):', this.buildDueDateAttachment());
        return;
      }
    ...
    try {
      ...
      if (parsed.action === 'assign_homework') {
        ...
        if (typeof parsed.dueDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(parsed.dueDate)) {
          ...
          if (d < today) {
            // Gemini gave a past date — advance year by year until it's in the future
  ...
  private parseRuDate(text: string): string | null {
    ...
    if (ru) { const m = RU_MONTHS[ru[2]]; if (m) return tryBuild(+ru[1], m, ru[3] ? +ru[3] : year); }
    const dot = t.match(/^(\d{1,2})\.(\d{1,2})(?:\.(\d{2,4}))?$/);
    if (dot) { let y = dot[3] ? +dot[3] : year; if (y < 100) y += 2000; return tryBuild(+dot[1], +dot[2], y); }
    const sl = t.match(/^(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?$/);
    if (sl) { let y = sl[3] ? +sl[3] : year; if (y < 100) y += 2000; return tryBuild(+sl[1], +sl[2], y); }
    return null;
  }

  private async doAssignHomework(chatId: string, userId: string, daysUntilDue: number, isoDate?: string): Promise<void> {
    const state = this.getMaxPlatformState(userId);
