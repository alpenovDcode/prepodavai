...
export class CalendarService {
  ...
  /**
   * Проверка пересечений: возвращает события того же пользователя,
   * перекрывающиеся с интервалом [startAt, endAt). Используется фронтом
   * перед созданием/переносом — учителю показывается предупреждение.
   * excludeId — id события, которое мы редактируем (его не считаем
   * пересечением с самим собой).
   */
  async findConflicts(
    userId: string,
    startIso: string,
    ...
    // Берём ближайшие события за +-1 неделю и считаем пересечение
    // на JS — это проще чем выражать RRULE-occurrences в SQL.
    const windowFrom = new Date(start.getTime() - 7 * 24 * 60 * 60 * 1000);
    const windowTo = new Date(end.getTime() + 7 * 24 * 60 * 60 * 1000);
    const events = await this.listEvents(userId, windowFrom.toISOString(), windowTo.toISOString());
    return events
      .filter((e: any) => {
        if (!e || e.legacy) return false;
        const masterId = e.masterId || e.id;
        if (excludeId && (e.id === excludeId || masterId === excludeId)) return false;
